...
- Sections/tabs (`section` state, set via the nav in `AcademiaPIR`): `curiosidades` (scrollable feed of AI-generated psychology trivia grounded in the temario, each card with an optional tap-to-reveal mini-question; auto-requests more from the server when the viewer's unseen pool runs low, and recycles already-seen cards once the whole pool has been seen), `perfil` (daily wheel/"ruleta", fail history, favorites — kept deliberately light), `logros` (level/badge grid + streak & accuracy stats, split out of `perfil` to declutter it), `simulacros` (self-test quizzes), `banco` (question bank — "Reales" curated + "Inventadas por IA" AI-generated, open to all users with a daily quota, save-to-bank admin-only), `duelo` (1v1 duel/battle mode), `ranking` (leaderboard + streaks). There is no `temario` tab — `TEMARIO` (static data in `src/temario.js`) is still imported and used internally to ground AI question generation (`GenerarPreguntasIA`) and AI trivia generation (`api/generar-curiosidades.js`), just not shown as a readable tab.
