...
const SEED_QUESTIONS = [
  { id: "q1", curso: "Psicopatología", tema: "Trastornos del estado de ánimo", pregunta: "Según los criterios diagnósticos clásicos, ¿cuál es la duración mínima de un episodio depresivo mayor?", opciones: ["1 semana", "2 semanas", "1 mes", "6 meses"], correcta: 1, explicacion: "El episodio depresivo mayor requiere síntomas presentes durante al menos dos semanas consecutivas." },
  { id: "q2", curso: "Psicopatología", tema: "Trastornos de ansiedad", pregunta: "El miedo intenso y súbito que alcanza su máxima expresión en minutos, acompañado de síntomas somáticos, define:", opciones: ["Fobia específica", "Crisis de angustia", "Trastorno de ansiedad generalizada", "Ansiedad de separación"], correcta: 1, explicacion: "La crisis de angustia (ataque de pánico) se caracteriza por un pico de malestar intenso en minutos." },
  { id: "q3", curso: "Evaluación psicológica", tema: "Instrumentos de evaluación", pregunta: "¿Qué mide fundamentalmente el MMPI en su versión clásica?", opciones: ["Inteligencia general", "Rasgos de personalidad y psicopatología", "Memoria de trabajo", "Funciones ejecutivas"], correcta: 1, explicacion: "El MMPI es un inventario multifásico de personalidad orientado a detectar patrones psicopatológicos." },
  { id: "q4", curso: "Evaluación psicológica", tema: "Fiabilidad y validez", pregunta: "La consistencia interna de un test se estima habitualmente mediante:", opciones: ["Validez de constructo", "Alfa de Cronbach", "Curva ROC", "Percentil"], correcta: 1, explicacion: "El alfa de Cronbach es el estadístico más usado para estimar consistencia interna." },
  { id: "q5", curso: "Psicología clínica", tema: "Terapias de tercera generación", pregunta: "La Terapia de Aceptación y Compromiso (ACT) tiene como objetivo central:", opciones: ["Eliminar por completo los pensamientos negativos", "Aumentar la flexibilidad psicológica", "Reestructurar creencias irracionales", "Extinguir respuestas condicionadas"], correcta: 1, explicacion: "ACT busca la flexibilidad psicológica: aceptar lo interno y actuar según valores." },
  { id: "q6", curso: "Psicología clínica", tema: "Terapia cognitivo-conductual", pregunta: "En la reestructuración cognitiva de Beck, los 'pensamientos automáticos' se caracterizan por ser:", opciones: ["Deliberados y reflexivos", "Involuntarios y creíbles para la persona", "Siempre conscientes", "Ajenos al estado de ánimo"], correcta: 1, explicacion: "Son pensamientos que surgen de forma involuntaria y se aceptan como verdaderos sin cuestionarlos." },
  { id: "q7", curso: "Psicología clínica", tema: "Trastornos de la conducta alimentaria", pregunta: "Un criterio distintivo entre anorexia nerviosa y bulimia nerviosa es:", opciones: ["La presencia de atracones", "El peso corporal significativamente bajo", "La preocupación por la imagen corporal", "El uso de purgas"], correcta: 1, explicacion: "El peso significativamente bajo para la edad y talla es criterio central de anorexia, no de bulimia." },
  { id: "q8", curso: "Neuropsicología", tema: "Funciones cognitivas", pregunta: "La incapacidad para reconocer caras familiares pese a una percepción visual conservada se denomina:", opciones: ["Afasia", "Apraxia", "Prosopagnosia", "Agnosia auditiva"], correcta: 2, explicacion: "La prosopagnosia es un déficit selectivo en el reconocimiento de rostros." },
  { id: "q9", curso: "Psicología social", tema: "Procesos grupales", pregunta: "La tendencia de un grupo a tomar decisiones más extremas que las que tomaría cada individuo por separado se llama:", opciones: ["Pensamiento grupal", "Polarización grupal", "Facilitación social", "Difusión de responsabilidad"], correcta: 1, explicacion: "La polarización grupal describe ese desplazamiento hacia posturas más extremas tras la deliberación." },
  { id: "q10", curso: "Psicología social", tema: "Influencia social", pregunta: "En el experimento clásico de Asch sobre conformidad, la variable manipulada principal era:", opciones: ["La intensidad del castigo", "El tamaño y unanimidad del grupo mayoritario", "El nivel de inteligencia del sujeto", "El tipo de tarea motriz"], correcta: 1, explicacion: "Asch variaba el tamaño del grupo y si había disidentes, para medir presión hacia la conformidad." }
];
...
async function loadShared(key, fallback) { try { const res = await window.storage.get(key, true); return res ? JSON.parse(res.value) : fallback; } catch { return fallback; } }
async function saveShared(key, value) { await window.storage.set(key, JSON.stringify(value), true); }
async function loadPersonal(key, fallback) { try { const res = await window.storage.get(key, false); return res ? JSON.parse(res.value) : fallback; } catch { return fallback; } }
async function savePersonal(key, value) { await window.storage.set(key, JSON.stringify(value), false); }
...
export default function AcademiaPIR() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setUser(u); setQuestions(q); setTemario(t); setRanking(r); setReady(true);
    })();
  }, []);
  ...
  const handleLogout = async () => { setUser(null); await savePersonal("pir-user", null); };
  ...
  return (
    <div style={styles.app}>
      ...
      <main style={styles.main}>
        ...
        {section === "banco" && <BancoPreguntas questions={questions} onAdd={addQuestion} />}
    ...
    <div style={{ ...styles.app, ...styles.center, minHeight: 520 }}>
      <div style={{ maxWidth: 340, width: "100%", padding: "0 24px", textAlign: "center" }}>
        ...
        <p style={{ color: "#5B6472", fontSize: 15, lineHeight: 1.5, marginBottom: 28 }}>
          Simulacros, banco de preguntas, temario y ranking en un mismo sitio. Decinos cómo te llamás para empezar.
        </p>
        <input value={nameInput} onChange={(e) => setNameInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onSubmit()} placeholder="Tu nombre" style={styles.input} autoFocus />
        <button onClick={onSubmit} style={{ ...styles.btnPrimary, width: "100%", marginTop: 12 }}>Entrar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <header style={styles.header}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 13, color: "#5B6472", display: "flex", alignItems: "center", gap: 4 }}><User size={14} /> {user.name}</span>
        <button onClick={onLogout} style={styles.iconBtn} title="Salir"><LogOut size={15} color="#5B6472" /></button>
      </div>
    </header>
  );
}
  ...
  return (
    <nav style={styles.nav}>
      {items.map((it) => {
        ...
        return (
          <button key={it.id} onClick={() => setSection(it.id)} style={{ ...styles.navBtn, color: active ? "#14213D" : "#8A93A3", borderBottom: active ? "2px solid #2E7D6B" : "2px solid transparent" }}>
      ...
      <div>
        ...
        <Card>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[5, 10, 20].map((n) => (
              <button key={n} onClick={() => setNumPreguntas(n)} style={{ ...styles.chip, background: numPreguntas === n ? "#14213D" : "transparent", color: numPreguntas === n ? "#fff" : "#14213D", borderColor: "#14213D" }}>{n}</button>
            ))}
          </div>
          <button onClick={start} style={{ ...styles.btnPrimary, width: "100%", marginTop: 22 }}>Empezar simulacro</button>
        </Card>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <Card style={{ marginTop: 16 }}>
          ...
          {q.opciones.map((op, i) => (
            <button key={i} onClick={() => choose(i)} style={{ ...styles.option, borderColor: selected === i ? "#14213D" : "#E4E1D8", background: selected === i ? "#EEF3F1" : "#fff" }}>{op}</button>
          ))}
          <button onClick={next} disabled={selected === null} style={{ ...styles.btnPrimary, width: "100%", marginTop: 18, opacity: selected === null ? 0.4 : 1 }}>
  ...
  return (
    <div>
      ...
      <Card style={{ textAlign: "center", padding: "32px 20px" }}>
        ...
        <button onClick={() => setState("config")} style={{ ...styles.btnSecondary, marginTop: 22 }}><RotateCcw size={14} style={{ marginRight: 6 }} /> Repetir simulacro</button>
      </Card>
...
function BancoPreguntas({ questions, onAdd }) {
  ...
  return (
    <div>
      <SectionTitle title="Banco de preguntas" subtitle={`${questions.length} preguntas disponibles`} action={<button onClick={() => setShowForm((s) => !s)} style={styles.btnSecondary}><Plus size={14} style={{ marginRight: 4 }} /> Añadir</button>} />
      ...
      <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "4px 0 14px" }}>
        {cursos.map((c) => (
          <button key={c} onClick={() => setFiltro(c)} style={{ ...styles.chip, whiteSpace: "nowrap", background: filtro === c ? "#14213D" : "transparent", color: filtro === c ? "#fff" : "#14213D", borderColor: "#14213D" }}>{c}</button>
        ))}
      </div>
      {filtered.map((q) => (<PreguntaCard key={q.id} q={q} />))}
    </div>
  );
}
        ...
        <div style={{ marginTop: 12 }}>
          {q.opciones.map((op, i) => (
            <div key={i} style={{ ...styles.option, cursor: "default", borderColor: i === q.correcta ? "#2E7D6B" : "#E4E1D8", background: i === q.correcta ? "#EEF3F1" : "#fff", display: "flex", alignItems: "center", gap: 8 }}>
              {i === q.correcta && <Check size={13} color="#2E7D6B" />}{op}
            </div>
          ))}
...
function Ranking({ ranking, user }) {
  ...
  return (
    <div>
      ...
      {sorted.length === 0 && (<Card style={{ textAlign: "center", color: "#8A93A3", padding: "28px 16px" }}>Todavía no hay resultados. Hacé un simulacro para aparecer acá.</Card>)}
      {sorted.map((r, i) => (
        <div key={i} style={{ ...styles.rankRow, background: r.name === user.name ? "#EEF3F1" : "#fff" }}>
...
function FieldLabel({ children, style }) { return <div style={{ fontSize: 12, color: "#5B6472", marginBottom: 6, ...style }}>{children}</div>; }

const styles = {
  app: { fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: "#FBF9F4", minHeight: "100%", color: "#14213D" },
