...
async function loadPersonal(key, fallback) { try { const raw = localStorage.getItem(key); return raw ? JSON.parse(raw) : fallback; } catch { return fallback; } }
async function savePersonal(key, value) { try { localStorage.setItem(key, JSON.stringify(value)); } catch {} }
...
export default function AcademiaPIR() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data: qData, error: qErr } = await supabase.from("preguntas").select("*").order("created_at", { ascending: true });
        if (qErr) throw qErr;
        const { data: rData, error: rErr } = await supabase.from("ranking").select("*").order("pct", { ascending: false }).limit(100);
        if (rErr) throw rErr;
        setUser(u);
        setQuestions(qData && qData.length ? qData : SEED_QUESTIONS);
        setTemario(SEED_TEMARIO);
        setRanking(rData || []);
        setReady(true);
      } catch (err) {
  ...
  const addQuestion = async (q) => {
    const { data, error } = await supabase.from("preguntas").insert([{
      curso: q.curso, tema: q.tema, pregunta: q.pregunta, opciones: q.opciones, correcta: q.correcta, explicacion: q.explicacion
    }]).select();
  ...
  const submitScore = async (entry) => {
    const { data, error } = await supabase.from("ranking").insert([{
      name: entry.name, score: entry.score, total: entry.total, pct: entry.pct, seconds: entry.seconds
    }]).select();
    if (!error && data && data[0]) setRanking((prev) => [...prev, data[0]].sort((a, b) => b.pct - a.pct).slice(0, 100));
  };
  ...
  return (
    <div style={{ ...styles.app, ...styles.center, minHeight: 520 }}>
      <div style={{ maxWidth: 340, width: "100%", padding: "0 24px", textAlign: "center" }}>
        ...
        <button type="button" onClick={onSubmit} style={{ ...styles.btnPrimary, width: "100%", marginTop: 12 }}>Entrar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <header style={styles.header}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <button type="button" onClick={onLogout} style={styles.iconBtn} title="Salir"><LogOut size={15} color="#5B6472" /></button>
      </div>
    </header>
  );
}
  ...
  return (
    <nav style={styles.nav}>
      {items.map((it) => {
        ...
        return (
          <button type="button" key={it.id} onClick={() => setSection(it.id)} style={{ ...styles.navBtn, color: active ? "#14213D" : "#8A93A3", borderBottom: active ? "2px solid #2E7D6B" : "2px solid transparent" }}>
...
function BancoPreguntas({ questions, onAdd }) {
  ...
  return (
    <div>
      <SectionTitle title="Banco de preguntas" subtitle={`${questions.length} preguntas disponibles`} action={<button type="button" onClick={() => setShowForm((s) => !s)} style={styles.btnSecondary}><Plus size={14} style={{ marginRight: 4 }} /> Añadir</button>} />
      ...
      <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "4px 0 14px" }}>
        {cursos.map((c) => (
          <button type="button" key={c} onClick={() => setFiltro(c)} style={{ ...styles.chip, whiteSpace: "nowrap", background: filtro === c ? "#14213D" : "transparent", color: filtro === c ? "#fff" : "#14213D", borderColor: "#14213D" }}>{c}</button>
        ))}
      </div>
