...
export default function AcademiaPIR() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: qData } = await supabase.from("preguntas").select("*").order("created_at", { ascending: true });
      const { data: rData } = await supabase.from("ranking").select("*").order("pct", { ascending: false }).limit(100);
      setUser(u);
